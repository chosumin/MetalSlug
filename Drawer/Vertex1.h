...
struct VertexPC : public Vertex
{
	...
	VertexPC(float x, float y, DWORD color, float depth = 0.f)
		:Vertex(x, y, depth)
	{
		this->color = color;
	}

	VertexPC(const Point& location, DWORD color, float depth = 0.f)
...
struct VertexPT : public Vertex
{
	...
	VertexPT(float x, float y, float u, float v, float depth = 0.f)
		: Vertex(x, y, depth)
	{
		coord = D3DXVECTOR2(u, v);
	}

	VertexPT(Point location, Point coord, float depth = 0.f)
		: Vertex(location, depth)
	{
